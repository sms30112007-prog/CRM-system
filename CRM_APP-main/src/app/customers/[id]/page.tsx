...
export default function CustomerDetailPage() {
  ...
  const handleLogActivity = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        setActivityDesc('');
        fetchCustomer(); // Refresh activities timeline
      }
    } catch (err) {
      console.error(err);
    } finally {
  ...
  const handleCreateTask = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setTaskDesc('');
        setTaskDueDate('');
        fetchCustomer(); // Refresh tasks list
      }
    } catch (err) {
      console.error(err);
    } finally {
  ...
  const handleDeleteCustomer = async () => {
    if (!confirm('Are you sure you want to delete this customer profile? This action is irreversible.')) {
      return;
    }
  ...
  return (
    <DashboardLayout>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-slate-400 text-xs mt-0.5">{customer.company || 'Private Account'}</p>
          </div>
        </div>
      ...
      {/* Main Grid: Details + Log Activity on Left, Tasks + Deals on Right */}
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 items-start">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 text-sm">
                <div className="space-y-4">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="text-[10px] font-semibold text-slate-500 uppercase tracking-wider">Account Owner</p>
                      <p className="text-slate-250 font-medium">{customer.name}</p>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-slate-250 font-medium">{customer.company || 'N/A'}</p>
                    </div>
                  </div>
                </div>
                ...
                <div className="space-y-4">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-indigo-400 hover:underline">{customer.email}</p>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-slate-250 font-medium">{customer.phone || 'N/A'}</p>
                    </div>
                  </div>
                </div>

                <div className="sm:col-span-2 flex items-center gap-3 pt-3 border-t border-slate-800/40">
                  <span className="text-[10px] font-semibold text-slate-500 uppercase tracking-wider">Assigned Rep:</span>
                  <span className="inline-flex items-center gap-1.5 px-2.5 py-0.5 bg-slate-900 border border-slate-800 text-xs font-medium text-slate-300 rounded-full">
                ...
                {customer.activities.map((act) => (
                  <div key={act.id} className="relative group">
                    ...
                    <div>
                      ...
                      <p className="text-slate-300 text-xs mt-1.5 leading-relaxed bg-slate-900/25 p-2.5 rounded-lg border border-slate-800/30">
                        {act.description}
                      </p>
                    </div>
                  </div>
        ...
        {/* Right Side: Tasks & Associated Deals (1 column) */}
        <div className="space-y-6">
          ...
          <div className="glass-panel rounded-2xl p-6 glow-card">
            ...
            <div className="space-y-2 mb-4">
              {customer.tasks.filter((t) => t.status !== 'COMPLETED').length === 0 ? (
                <p className="text-[11px] text-slate-500 py-2">No pending tasks for this account.</p>
              ) : (
                customer.tasks
            ...
            <form onSubmit={handleCreateTask} className="border-t border-slate-800/80 pt-4 space-y-3">
              <p className="text-[10px] font-semibold text-slate-400 uppercase tracking-wider">Schedule New Task</p>
              <input
                type="text"
                required
                value={taskTitle}
                onChange={(e) => setTaskTitle(e.target.value)}
                placeholder="Callback, follow up, demo..."
          ...
          <div className="glass-panel rounded-2xl p-6 glow-card">
            ...
            {customer.deals.length === 0 ? (
              <p className="text-xs text-slate-500 py-2">No deals linked to this account.</p>
            ) : (
              <div className="space-y-3">
                {customer.deals.map((deal) => (
                  <div
                    key={deal.id}
                    className="p-3 bg-slate-900/40 border border-slate-800/80 rounded-xl flex items-center justify-between gap-4"
                  >
                    <div>
                      <p className="text-xs font-bold text-slate-200">{deal.title}</p>
                      <span className="inline-block mt-1 text-[9px] font-bold px-1.5 py-0.5 bg-indigo-950/40 text-indigo-400 border border-indigo-900/40 rounded">
                        {deal.stage}
                      </span>
                    </div>
